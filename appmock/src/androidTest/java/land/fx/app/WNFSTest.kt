...
class WNFSTest {
    class MockFulaClient(val sharedPreferences: SharedPreferences): Client{
        fun ByteArray.toHexString() = joinToString("") { "%02x".format(it) }
        override fun put(data: ByteArray, codec: Long): ByteArray{
            val codecObj = Cid.Codec.lookup(codec)
            val digest: MessageDigest = MessageDigest.getInstance("SHA-256")
            val hash: ByteArray = digest.digest(data)
            val cid = Cid.buildCidV1(codecObj, Multihash.Type.sha2_256, hash)
            val base64Data = Base64.encodeToString(data, Base64.DEFAULT);
            sharedPreferences.edit().putString(cid.toString(), base64Data).commit()
            return cid.toBytes()
        }
        override fun get(cid: ByteArray): ByteArray{
            val cid = Cid.cast(cid)
            val base64Data = sharedPreferences.getString(cid.toString(), "")
...
//        val client = Fulamobile.newClient(configExt)
        val client = MockFulaClient(PreferenceManager.getDefaultSharedPreferences(appContext))
